...
Concept SyntacticElementFactoryImpl::make_thesis_concept(const Concept& concept_left, const Role& role, const Concept& concept_right, const Concept& concept_painted) {
    return Concept(m_vocabulary_info, m_caches.m_concept_cache->insert(
        std::make_unique<element::ThesisConcept>(*m_vocabulary_info, concept_left.get_element(), role.get_element(), concept_right.get_element(), concept_painted.get_element())).first);
}
