...
inline bool is_strips_fact(const FactProxy &fact_proxy) {
    return is_strips_fact(fact_proxy.get_name());
}

extern std::vector<FactPair> get_strips_fact_pairs(const TaskProxy &task_proxy);
