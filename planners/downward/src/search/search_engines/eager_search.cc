...
EagerSearch::EagerSearch(const Options &opts)
    : SearchEngine(opts),
      ...
      policy_lookahead_limit(opts.get<int>("policy_lookahead_limit")),
      ...
      preferred_operators_policy(opts.get<std::shared_ptr<policy::Policy>>("preferred_operators_policy", nullptr)),
      rng(utils::parse_rng_from_options(opts)),
      num_preferred_states(0),
      num_lookaheads(0) {
...
void EagerSearch::print_statistics() const {
    ...
    utils::g_log << "States reached via preferred operators: " << num_preferred_states << endl;
    utils::g_log << "Policy lookaheads: " << num_lookaheads << endl;
}
...
SearchStatus EagerSearch::step() {
    ...
    for (int i = 0; i <= policy_lookahead_limit; ++i) {
        ...
        // Note: since we only add new nodes to preferred_nodes, we automatically
        // stop if the policy loops.
        SearchStatus status = expand(*node, preferred_nodes);
        if (status == SOLVED) {
            return SOLVED;
        }
...
SearchStatus EagerSearch::expand(
    const SearchNode &node, vector<SearchNode> &preferred_nodes) {
    ...
    */
    ...
    // Lookahead: avoid always choosing the first operator that matches the policy.
    ...
    for (OperatorID op_id : applicable_ops) {
        ...
        bool pref_h = preferred_operators.contains(op_id);
        bool pref_policy = false;
        /* Query policy when computing preferred operators or if we haven't
           found a preferred successor for the lookahead yet. */
        if (preferred_operators_policy && (policy_lookahead_limit == 0 || preferred_nodes.empty())) {
            pref_policy = preferred_operators_policy->evaluate_effects_lazy(s,succ_state,rules);
        }
        ...
        if (succ_node.is_new()) {
            ...
            if (pref_policy && preferred_nodes.empty()) {
                preferred_nodes.push_back(succ_node);
            }
...
void add_options_to_parser(OptionParser &parser) {
    parser.add_option<std::shared_ptr<policy::Policy>>(
        "preferred_operators_policy",
        "policy for marking operators preferred",
        OptionParser::NONE);
    parser.add_option<int>(
        "policy_lookahead_limit",
        "maximum number of steps to follow the policy",
        "0",
        Bounds("0", "infinity"));
